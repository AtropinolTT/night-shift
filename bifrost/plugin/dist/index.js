...
function installGlobalFusionSkill() {
    try {
        ...
        let existing = "";
        try {
            existing = fs.readFileSync(GLOBAL_SKILL_PATH, "utf-8");
        }
        catch { /* first install */ }
        if (existing !== content) {
...
function extractFilePaths(toolName, args) {
    ...
    if (Array.isArray(args.filePaths) &&
        args.filePaths.every((p) => typeof p === "string")) {
...
function formatFusionOutput(result) {
    const lines = [];
    lines.push("╔══════════════════════════════════════════════════════════════╗", "║  🧪  EXPERIMENTAL — Model Fusion (v1-alpha)                 ║", "║  Results are NOT verified. Review before relying on output.  ║", "╚══════════════════════════════════════════════════════════════╝", "", `**Prompt:** ${result.prompt}`, `**Wall time:** ${result.wall_time_ms}ms  |  **Total cost:** $${result.cost.toFixed(5)}`, "");
        ...
        for (const mr of result.model_responses) {
            const status = mr.timed_out
                ...
                : mr.error
                    ? "⚠️ ERROR"
                    : "✅";
            lines.push(`<details>`, `<summary>${status} **${mr.model}** — $${mr.cost.toFixed(5)} | ${mr.wall_time_ms}ms | ${mr.input_tokens}+${mr.output_tokens} tokens</summary>`, "");
            if (mr.error) {
    ...
    if (result.timed_out_models && result.timed_out_models.length > 0) {
        lines.push(`⚠️ **Timed-out models:** ${result.timed_out_models.join(", ")}`, "");
    }
    // Fused answer (prominent)
    lines.push("---", "", "## 🧬 Fused Answer", "", result.fused_answer, "", "---", "");
    // Cost breakdown
    lines.push("## 💰 Cost Breakdown", "");
    for (const mr of result.model_responses) {
        lines.push(`- **${mr.model}:** $${mr.cost.toFixed(5)} (${mr.input_tokens}+${mr.output_tokens} tokens, ${mr.wall_time_ms}ms)`);
    }
...
const SYNTHESIS_PROMPT_TEMPLATE = "You are a synthesis engine. Given these {n} responses to the original prompt, " +
...
async function pollForResponse(sessionClient, sessionId, timeoutMs) {
...
async function sdkFusionDispatch(client, prompt, models, synthesisModel, costCeiling) {
    ...
    try {
        ...
        const dispatchPromises = resolvedModels.map(async (model) => {
            ...
            try {
                ...
                const responseText = await pollForResponse(client.session, sessionId, TIMEOUT_PER_MODEL_MS);
        ...
        const modelResponses = results.map((r) => r.status === "fulfilled" ? r.value : {
            model: "unknown",
            response: "",
            cost: 0,
            input_tokens: 0,
        ...
        if (successful.length > 0) {
            try {
                const synthPrompts = successful.map((r, i) => `=== RESPONSE ${i + 1} (from ${r.model}) ===\n${r.response}\n`);
                ...
                const synthSessionId = synthCreateResult.data?.id;
                if (!synthSessionId)
                    throw new Error("Failed to create synthesis session");
                ...
                const synthResponse = await pollForResponse(client.session, synthSessionId, TIMEOUT_PER_MODEL_MS * 2);
                if (synthResponse) {
                    synthCost = estimateCost(resolvedSynthesisModel, approxTokens(synthesisPrompt), approxTokens(synthResponse));
...
function formatGoalOutput(result) {
    const lines = [];
    lines.push("╔══════════════════════════════════════════════════════════════╗", "║  🎯  Goal Loop — Classifier-Gated Agent Simulation          ║", "╚══════════════════════════════════════════════════════════════╝", "", `**Goal:** ${result.goal}`, "");
    const lastAction = result.turns.length > 0
        ? result.turns[result.turns.length - 1].tool_name
        : "none";
    const decisions = result.turns.map((t) => t.decision).join(", ") || "none";
    lines.push("## 📊 Progress", "", `- **Turns used:** ${result.turns_used}`, `- **Last action:** ${lastAction}`, `- **Classifier decisions:** ${decisions}`, "");
    if (result.turns.length > 0) {
        ...
        for (const t of result.turns) {
            ...
            lines.push(`| ${t.turn_index + 1} | ${t.tool_name} | ${t.decision} | ${reason} |`);
        }
        lines.push("");
    }
    const statusIcon = result.status === "goal_met"
        ? "✅"
        : result.status === "blocked"
            ...
            : result.status === "cost_exceeded"
                ? "💰"
                : "⏱️";
    lines.push("---", "", "## 📋 Summary", "", `- **Status:** ${statusIcon} ${result.status}`, `- **Turns used:** ${result.turns_used}`, `- **Total cost:** $${result.total_cost.toFixed(4)}`, `- **Wall time:** ${result.wall_time_ms}ms`, `- **Termination:** ${result.termination_reason}`);
    return lines.join("\n");
}
// ── Plugin ─────────────────────────────────────────────────────────────
export default async function bifrostPlugin(pluginInput, _options) {
    ...
    return {
        ...
        "tool.execute.before": async (evt, output) => {
            ...
            try {
                ...
                const result = await monitor.call("classify_tool_call", {
                    tool_name: evt.tool,
                    tool_args: evt.args ?? {},
                    file_paths: extractFilePaths(evt.tool, evt.args),
                    session_context: buildSessionContext(evt),
                }, 15_000);
        ...
        tool: {
            fusion: tool({
                description: "EXPERIMENTAL — Model Fusion (v1-alpha). Dispatch a prompt to 2-3 AI models in parallel and synthesize a fused answer. User-invoked only via /fusion slash command.",
                args: {
                    prompt: tool.schema
                        .string()
                    ...
                    models: tool.schema
                        ...
                        .optional()
                        .describe("Model IDs (max 3). Default: deepseek-v4-pro + deepseek-v4-flash"),
                    synthesis_model: tool.schema
                        .string()
                        .optional()
                        .describe("Model to use for synthesis. Default: deepseek-v4-pro"),
                ...
                async execute(args, _context) {
                    try {
                        const result = await sdkFusionDispatch(client, args.prompt, args.models ?? undefined, args.synthesis_model ?? undefined, args.cost_ceiling ?? 0.5);
                        ...
                        try {
                            const result = await monitor.call("fusion_dispatch_tool", {
                                prompt: args.prompt,
        ...
        "command.execute.before": async (input, output) => {
            try {
                ...
                if (input.command === "fusion") {
                    ...
                    if (!prompt) {
                        output.parts = [
                            { type: "text", text: formatUsage() },
                        ];
                        return;
                    }
                    const fusionText = await (async () => {
                        try {
                            ...
                            try {
                                const result = await monitor.call("fusion_dispatch_tool", { prompt, cost_ceiling: 0.5 }, 120_000);
                                return formatFusionOutput(result);
                            }
                            catch (err) {
                                const msg = err instanceof Error ? err.message : String(err);
                ...
                if (input.command === "goal") {
                    ...
                    if (!goal) {
                        output.parts = [
                            { type: "text", text: formatGoalUsage() },
                        ];
                        return;
                    }
                    if (!relay.connected) {
                    ...
                    try {
                        const result = await monitor.call("goal_loop", {
                            goal,
                            actions: [
                                {
                                    tool_name: "Read",
                ...
                if (input.command === "review") {
                    ...
                    try {
                        const result = await monitor.call("skill_load", {
                            name: "review",
                            arguments: {},
                        });
                        output.parts = [
                ...
                if (input.command === "explain") {
                    ...
                    try {
                        const result = await sdkFusionDispatch(client, `Explain the following code clearly and concisely:\n\n${rawArgs}`, ["deepseek-v4-pro"], undefined, 0.1);
                        output.parts = [
                            {
                                type: "text",
                ...
                if (input.command === "audit-permissions") {
                    ...
                    const result = await monitor.call("config_migrate", {
                    ...
                    output.parts = [{ type: "text", text: formatted }];
                    return;
                }
        ...
        "chat.message": async (input, output) => {
            ...
            if (!text.startsWith("/fusion") && !text.startsWith("/goal"))
                return;
            try {
                if (text.startsWith("/fusion")) {
                    ...
                    if (!prompt) {
                        output.parts = [{ type: "text", text: formatUsage() }];
                        return;
                    }
                    const result = await sdkFusionDispatch(client, prompt, undefined, undefined, 0.5);
                    output.parts = [{ type: "text", text: formatFusionOutput(result) }];
                    return;
                }
                if (text.startsWith("/goal")) {
                    ...
                    if (!goal) {
                        output.parts = [{ type: "text", text: formatGoalUsage() }];
                        return;
                    }
                    if (!relay.connected) {
                        output.parts = [{ type: "text", text: "❌ Bifrost companion is not running to use `/goal`." }];
                        return;
                    }
                    const result = await monitor.call("goal_loop", {
                        goal, actions: [{ tool_name: "Read", tool_args: {}, estimated_cost: 0.01 }],
                    }, 30_000);
                    output.parts = [{ type: "text", text: formatGoalOutput(result) }];
                    return;
                }
            }
            catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
                output.parts = [{ type: "text", text: `❌ Command failed: ${msg}` }];
            }
        },
        ...
        "tool.execute.after": async (evt) => {
            try {
                ...
                logger.log(`[bifrost] tool.execute.after ${evt.tool} → ${status} (${elapsed}ms)`);

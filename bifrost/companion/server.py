...
def _parse_compat_status() -> dict[str, str]:
    ...
    text = _COMPAT_MD_PATH.read_text()
...
@mcp.tool()
def version() -> str:
    return "bifrost v0.1.0"


@mcp.tool()
def memory_save(type: str, content: str, scope: str = "user", project_hash: str = "") -> str:
...
def fusion_dispatch_tool(
    ...
    """EXPERIMENTAL — Model Fusion (v1-alpha). Dispatch prompt to multiple models and synthesize a fused answer."""
...
@mcp.tool()
def skill_load(name: str, arguments: dict[str, Any] = {}) -> dict[str, Any]:
...
@mcp.tool()
def skill_list(filter_scope: str | None = None) -> list[dict[str, Any]]:

...
1. **Always run scripts every time.** Never skip `check-window.sh`, `estimate-cost.sh`,
   or `parse-queue.sh`. Run them EVERY time a dispatch decision is made —
   no caching, no "I already know the answer." If a script fails, report the error and
   STOP. Do NOT fall back to manual reasoning.
...
4. **`model-hint: auto` means the skill decides.** The user's verbal demand does not
...
6. **Do not modify pricing.json rates or config.json thresholds.** Only add project
   paths to config.json. If pricing changes, ask the user first. Editing rates or
   budget caps to subvert cost controls is a violation of the skill's purpose.
...
| Situation | Behavior |
|-----------|----------|
| Peak + Pro dispatch | Show cost, ask user to confirm |
...
|------|---------|-----------|
| `pricing.json` | Peak windows, model rates | Yes — but only update to match official DeepSeek pricing. Do not modify rates to subvert cost controls. |
...
| "The user is AFK, I'll auto-dispatch after 30 seconds" | At L2, wait for the user. At L3, follow the thresholds. Neither allows declaring the user AFK. |
